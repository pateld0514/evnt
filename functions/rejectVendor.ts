...
Deno.serve(async (req) => {
  try {
    ...
    if (!vendorId || !userId || !reason) {
      return Response.json({ error: 'vendorId, userId, and reason are required' }, { status: 400 });
    }
    ...
    await base44.asServiceRole.entities.User.update(userId, { approval_status: "rejected" });
    ...
    return Response.json({ error: error.message || 'Failed to reject vendor' }, { status: 500 });
  }
});